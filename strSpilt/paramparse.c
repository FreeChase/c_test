 ...
 * 
 * @param string 字符串参数
 * @return unsigned long long 解析出的数字
 */
static unsigned long long paramsParseNumber(char *string)
{
    ShellNumType type = NUM_TYPE_DEC;
    char radix = 10;
    char *p = string;
    ...
    if (type == NUM_TYPE_FLOAT && devide != 0)
    {
        ...
        return *(unsigned long long *)(&valueFloat);
    }
    else
    {
 ...
 * 
 * @param string 参数
 * @return unsigned long long 解析结果
 */
unsigned long long paramsParsePara(char *string)
{
    if (*string == '\'' && *(string + 1))
    {
        return (unsigned long long)paramsParseChar(string);
    }
    else if (*string == '-' || (*string >= '0' && *string <= '9'))
    {
        return (unsigned long long)paramsParseNumber(string);
    }
    else if (*string)
    {
        return (unsigned long long)paramsParseString(string);
    }
    return 0;
}
