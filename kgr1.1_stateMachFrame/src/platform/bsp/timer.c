 ...
 */
#define RESET_VALUE	 (XPAR_CPU_M_AXI_DP_FREQ_HZ / 1000 )//0xf0000000
			...
			u8 TmrCtrNumber)
{
	...
	while (1) {
		 ...
		 */
		while (gTimerTick == LastTimerExpired) {
		}
		LastTimerExpired = gTimerTick;
...
int timerInit(void)
{
	 ...
	 */
	Status = TmrCtrIntrExample(&IntcInstancePtr,
				  &TimerCounterInst,
				  TMRCTR_DEVICE_ID,
				  TMRCTR_INTERRUPT_ID,
				  TIMER_CNTR_0);

	xil_printf("TimerInit Over!\r\n");
	return XST_SUCCESS;
