...
int flash_wear_level_init() {
    ...
    for (uint32_t i = 0; i < NUM_LOGICAL_BLOCKS; i++) {
        uint32_t block_address = WEAR_LEVEL_AREA_START + (i * FLASH_SECTOR_SIZE);
...
int flash_wear_level_write(ProductData_t *data) {
    uint32_t target_address = WEAR_LEVEL_AREA_START + (current_block_index * FLASH_SECTOR_SIZE);
...
void run_test_case_4_corrupted_block_handling() {
    ...
    printf("\n--- Manually Corrupting Block 1 (address 0x%X) ---\n", WEAR_LEVEL_AREA_START + (1 * FLASH_SECTOR_SIZE));
    uint32_t corrupt_addr = WEAR_LEVEL_AREA_START + (1 * FLASH_SECTOR_SIZE);
...
void run_test_case_5_all_blocks_corrupted() {
    ...
    for (int i = 0; i < NUM_LOGICAL_BLOCKS; i++) {
        uint32_t corrupt_addr = WEAR_LEVEL_AREA_START + (i * FLASH_SECTOR_SIZE);
