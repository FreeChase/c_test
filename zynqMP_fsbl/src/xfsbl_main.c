...
#error "FSBL should be generated using only EL3 BSP"
	...
	while (FsblStage<=XFSBL_STAGE_DEFAULT) {
		...
		case XFSBL_STAGE2:
			{
						...
						(XFSBL_STATUS_JTAG != FsblStatus) )
				{
					 ...
					 */
					FsblStage = XFSBL_STAGE3_1;//XFSBL_STAGE4;
				} else {
					...
					Xil_Out32(XFSBL_R5_USAGE_STATUS_REG,
						  ...
						  ~(XFSBL_R5_0_STATUS_MASK |
						    XFSBL_R5_1_STATUS_MASK)));
					if( FsblInstance.ImageHeader.ImageHeaderTable.NoOfPartitions ==  3)
					{
						//*normal image
		...
		case XFSBL_STAGE3:
			{
				...
				if (XFSBL_SUCCESS != FsblStatus)
				{
						...
						(FsblInstance.ImageHeader.ImageHeaderTable.NoOfPartitions-1U))
					{
						if (TRUE == FsblStatus) {
							EarlyHandoff = TRUE;
							FsblStage = XFSBL_STAGE3_0_A;//XFSBL_STAGE4;//XFSBL_STAGE3_1;
						}
						else {
							/**
		...
		case XFSBL_STAGE3_0_A:
		case XFSBL_STAGE3_0_B:
			{
				PartitionNum = 0;
				if(FsblStage == XFSBL_STAGE3_0_A)
				{
					u32FlashLoadAddr = addrTable[eFirewareImage_PLBit].u32addr;
				}
				else
				{
					u32FlashLoadAddr = addrTable[eFirewareImage_AppA53].u32addr;
				}
				FsblStatus =  XImage_ValidateHeader(&FsblInstance, u32FlashLoadAddr);
				if (XFSBL_SUCCESS != FsblStatus)
				{
					// FsblStatus += XFSBL_ERROR_STAGE_5;
					gimageloadflag |= (1<<FsblStage);
					//* if load fail, jump to XFSBL_STAGE3_1 directly
					FsblStage = XFSBL_STAGE3_1;
					break;
				}

				XFsbl_Printf(DEBUG_INFO,
					"======= In Stage 3_0_A, Partition No:%d ======= \n\r",
					PartitionNum);
				...
				if (XFSBL_SUCCESS != FsblStatus)
				{
					 ...
					 * Error
					 */
					XFsbl_Printf(DEBUG_GENERAL,"stage %d Partition %d Load Failed, 0x%0lx\n\r",
							FsblStage,PartitionNum, FsblStatus);
					FsblStatus += XFSBL_ERROR_STAGE_3;
					FsblStage = XFSBL_STAGE3_1;
				} else {
					XFsbl_Printf(DEBUG_INFO,"Partition %d Load Success \n\r",
					...
					if (TRUE == FsblStatus) {
						EarlyHandoff = TRUE;
						// FsblStage = XFSBL_STAGE3_1;//XFSBL_STAGE4;//XFSBL_STAGE3_1;
						if(XFSBL_STAGE3_0_A == FsblStage)
						{
							FsblStage = XFSBL_STAGE3_0_B;
						}
						else
						{
							FsblStage = XFSBL_STAGE3_1;
						 ...
						 * Load the next partition
						 */
						// xil_printf("Error: havnt app image!!!\r\n");
						// FsblStage = XFSBL_STAGE3_1;
						if(XFSBL_STAGE3_0_A == FsblStage)
						{
							FsblStage = XFSBL_STAGE3_0_B;
